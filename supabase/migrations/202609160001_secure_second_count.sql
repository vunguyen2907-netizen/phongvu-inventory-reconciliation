...
-- Counter-facing read/write contract. Counters never query protected evidence
-- or secrets directly: this RPC returns only the masked task projection.
...
begin
  ...
  select * into v_task from public.recount_tasks t
  where t.id = p_task_id and t.assigned_user_id = v_actor.id for update;
  if not found then
  ...
  if v_secret.expected_serial_normalized is not null
    ...
    v_result := 'matched'; v_resolution := 'matched'; v_state := 'completed';
    ...
    v_result := 'matched'; v_resolution := 'genuine_surplus'; v_state := 'completed';
  elsif exists (
    select 1 from public.recount_serial_evidence e
    where e.batch_id = v_task.batch_id and e.serial_normalized = v_scan and not e.is_excluded
  ) then
    select exists (
      select 1 from public.recount_serial_evidence e
    ...
    v_result := case when v_same_sku then 'duplicate_first_count' else 'wrong_sku' end;
    v_resolution := case when v_same_sku then 'mistaken_first_scan' else 'not_found' end;
    v_state := 'in_progress';
  else
    v_result := 'unknown_serial'; v_resolution := 'not_found'; v_state := 'in_progress';
  end if;

  insert into public.recount_attempts (
    task_id, user_id, user_name_snapshot, scanned_value_hash,
    scanned_value_masked, result
  ) values (v_task.id, v_actor.id, v_actor.full_name, v_hash, v_masked, v_result);
  ...
  return jsonb_build_object(
    'task_id', v_task.id, 'result', v_result, 'masked_value', v_masked,
    'state', v_state,
    'resolution', case when v_state = 'completed' then v_resolution else null end
  );
end;
